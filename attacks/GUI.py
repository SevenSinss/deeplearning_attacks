        ...
        try:
            ...
            if st.button("隐写并发送"):
                with st.spinner("正在隐写中，请稍等..."):
                    ...
                    # 保存秘密图像
                    output_c_path = os.path.join(base_dir, "add_secret.png")
                    output_C_PIL = transforms.ToPILImage()(output_c_tensor.squeeze().cpu().data / 2 + 0.5).convert('RGB')
                    try:
                        output_C_PIL.save(output_c_path, format="PNG")
                        logger.info(f"隐写图像已保存至 {output_c_path}")
                    except Exception as e:
...
# 主动隐写防火墙页面
    ...
    if st.button("删除攻击"):
        ...
        if not os.path.exists(output_c_path):
            st.error(f"隐写图像 '{output_c_path}' 不存在，请先运行发送方隐写操作！")
        else:
            try:
                # 初始化进度条和计时器
                ...
                # 打开隐写了秘密的图像
                output_C_PIL = Image.open(output_c_path).convert('RGB')
        ...
        if uploaded_t_file is None:
            ...
            if not os.path.exists(output_c_path):
                ...
                try:
                    ...
                    # 保存并显示结果图像
                    c_attack_path = os.path.join(base_dir, "C_attack.png")
                    s_attack_path = os.path.join(base_dir, "S_attack.png")
                    C_attack_PIL_2 = transforms.ToPILImage()(C_attack_tensor_2.squeeze().cpu().data / 2 + 0.5).convert('RGB')
                    S_attack_PIL_2 = transforms.ToPILImage()(S_attack_tensor_2.squeeze().cpu().data / 2 + 0.5).convert('RGB')
                    try:
                        C_attack_PIL_2.save(c_attack_path, format="PNG")
                        logger.info(f"容器图像已保存至 {c_attack_path}")
                    except Exception as e:
                        st.error(f"保存容器图像失败：{str(e)}")
                        logger.error(f"保存容器图像失败：{str(e)}")
                        st.stop()
                    try:
                        S_attack_PIL_2.save(s_attack_path, format="PNG")
                        logger.info(f"秘密图像已保存至 {s_attack_path}")
                    except Exception as e:
